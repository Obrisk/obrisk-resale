(function() {
  ...
  (function(exports) {
    ...
    function Options(opts) {
      ...
      this.defaultProtocol = opts.hasOwnProperty("defaultProtocol")
        ? opts.defaultProtocol
        : defaults.defaultProtocol;
      this.events = opts.hasOwnProperty("events")
        ? opts.events
        : defaults.events;
      this.format = opts.hasOwnProperty("format")
        ? opts.format
        : defaults.format;
      this.formatHref = opts.hasOwnProperty("formatHref")
        ? opts.formatHref
        : defaults.formatHref;
      this.nl2br = opts.hasOwnProperty("nl2br") ? opts.nl2br : defaults.nl2br;
      this.tagName = opts.hasOwnProperty("tagName")
        ? opts.tagName
        : defaults.tagName;
      this.target = opts.hasOwnProperty("target")
        ? opts.target
        : defaults.target;
      this.validate = opts.hasOwnProperty("validate")
        ? opts.validate
        : defaults.validate;
      this.ignoreTags = [];
      ...
      this.attributes =
        opts.attributes || opts.linkAttributes || defaults.attributes;
      this.className = opts.hasOwnProperty("className")
        ? opts.className
        : opts.linkClass || defaults.className;

      // Make all tags names upper case
      var ignoredTags = opts.hasOwnProperty("ignoreTags")
        ? opts.ignoreTags
        : defaults.ignoreTags;
    ...
    Options.prototype = {
      ...
      get: function get(key, operator, token) {
        ...
        switch (typeof option === "undefined" ? "undefined" : _typeof(option)) {
          ...
          case "object":
            optionValue = option.hasOwnProperty(token.type)
              ? option[token.type]
              : defaults[key];
            return typeof optionValue === "function"
              ? optionValue(operator, token.type)
              : optionValue;
        }
 ...
 */
    ...
    var partialProtocolMailtoStates = stateify(
      "mailto",
      S_START,
      DOMAIN,
      DOMAIN
    ...
    // Localhost
    var partialLocalhostStates = stateify(
      "localhost",
      S_START,
      LOCALHOST,
      DOMAIN
 ...
 */
    ...
    S_MAILTO_EMAIL.on(qsAccepting, S_MAILTO_EMAIL).on(
      qsNonAccepting,
      S_MAILTO_EMAIL_NON_ACCEPTING
    );
    S_MAILTO_EMAIL_NON_ACCEPTING.on(qsAccepting, S_MAILTO_EMAIL).on(
      qsNonAccepting,
      S_MAILTO_EMAIL_NON_ACCEPTING
    );
...
$(document).ready(function() {
  ...
  */
      ...
      function linkifyElementHelper(element, opts, doc) {
        ...
        if (
          element.tagName === "A" ||
          options.contains(ignoreTags, element.tagName)
        ) {
          // No need to linkify
          return element;
        }
        ...
        while (childElement) {
          ...
          switch (childElement.nodeType) {
            ...
            case TXT_NODE: {
              ...
              if (
                tokens.length === 0 ||
                (tokens.length === 1 && tokens[0] instanceof TEXT_TOKEN)
              ) {
